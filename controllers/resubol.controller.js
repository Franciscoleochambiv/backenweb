...
const Itemsbol = require('../models/Itemsbol');
const Resbol = require('../models/Resbol');
     ...
     var fs = require('fs');
      ...
      horaA=f.getFullYear()+"-"+(f.getMonth() +1)+"-"+f.getDate();
               ...
               // var rucemisor="10309611131";
                ...
                var nombrearchivo=rucemisor+"-RC-"+fecharchivo+"-"+req.body.numero;
               ...
               // createInvoice(invoice, pdf);
                ...
                fs.writeFileSync(archivosinfirma, plantilla1);
               ...
               // serie=data.items[atr].dvc_serie;
                ...
                for(var atr in data.itemsAnu){
                   ...
                   // serie=data.items[atr].dvc_serie;
                    ...
                    var plantilla24='<sac:SummaryDocumentsLine>'+
                        '<cbc:LineID>'+item.toString()+'</cbc:LineID>'+
                        '<cbc:DocumentTypeCode>'+tipo+'</cbc:DocumentTypeCode>'+
                        '<cbc:ID>'+nrobol+'</cbc:ID>'+
                        '<cac:AccountingCustomerParty>'+
                ...
                var plantilla33 ='</SummaryDocuments>';
                //</SummaryDocuments>';
                ...
                console.log("llenado de plantilla 33 cierre del archivo");
                ...
                function (error, stdout, stderr) {
                    ...
                    // Imprimimos en pantalla con console.log
                     ...
                     var data = fs.readFileSync(firmadozip,{
                        ...
                        if(err){
                            console.log("ha ocurrido un arror firmando el archivo ");
                            console.error(err)
                        }
                        else{
                         ...
                         // console.log()
                            ...
                            client.sendSummary({
                              ...
                              if(err){
                                  ...
                                  var mo= Resbol.findOne({title:title},function callback(error,a){
                                      if (error !==null){
                                      ...
                                      console.log(a);
                                       ...
                                       //console.log(res.applicationResponse);
                                        ...
                                        console.log(contenido);
                                         var actualizalo= Resbol.findByIdAndUpdate(idmodifica, {
                                           error:contenido
...
/*
  ...
  await Note.findByIdAndUpdate(req.params.id, {
      ...
      author
                                         ...
                                         var mo= Resbol.findOne({title:title},function callback(error,a){
                                             ...
                                             if (a==null){
                                              ...
                                              console.log(res.ticket);
                                               ...
                                               console.log(contenido);
                                                var actualiza= Resbol.findByIdAndUpdate(idmodifica, {
                                                  content:contenido,
                                                  xml:data,
                                                  cdr:"0"
                ...
                }//aumentado para el esle de stderror

                }); //fin del pŕocedimiento de firma desde el sistema
...
resbolCtrl.getNote = async (req, res) => {
    const note = await Resbol.findById(req.params.id);
    res.json(note);
}
...
resbolCtrl.getNoteope = async (req, res) => {
    const note1 = await Resbol.findOne({title:req.params.idope}, function(err,note) {
...
resbolCtrl.deleteNote = async (req, res) => {
    await Resbol.findByIdAndDelete(req.params.id)
...
resbolCtrl.updateNote = async (req, res) => {
    const { title, content, duration, date, author } = req.body;
    await Resbol.findByIdAndUpdate(req.params.id, {
        title,
        content,
        duration,
        author
...
resbolCtrl.deleNota = async (req, res) => {
    const note = await Resbol.findOne({title:req.params.idope}, function(err,borrado) {
        if(err) {
            return console.log(err);
        }
           console.log(borrado);
